...
class CategoricalOrdinalEnc(EncBase):
    ...
    def init_with_sorted_values(self, sorted_vals):
        self._item_to_idx = {item: idx for idx, item in enumerate(sorted_vals, start=1)}
        self._item_to_idx[None] = 0
        self._item_to_idx[np.nan] = 0
        self.cat_cards = [len(set(self._item_to_idx.values()))]

    def enc_cat(self, data):
        """
        Values that the encoder has never seen before are returned as 0
        """
        data = self.clean_data(data)
        idxs = [self._item_to_idx.get(item, 0) for item in data]
...
class ScalarQuantileOrdinalEnc(EncBase):
    ...
    def enc_cat(self, data):
        """
        Missing values are returned as category 0
        """
        ...
        if None in data:
            ...
            if len(val_idxs) > 0:
                ...
                idxs[val_idxs] = vals + 1
            idxs[null_idxs] = 0
        else:
            idxs = self.disc.transform(data).reshape(-1)
        return torch.LongTensor(idxs).unsqueeze(1)
...
class LatLongQuantileOrdinalEnc(EncBase):
    def __init__(self, disc_params=None):
        if disc_params is not None:
            ...
            for disc, (n_bins_, bin_edges_) in zip(self.discs, disc_params):
                ...
                self.cat_cards.append(n_bins_ + 1)
    ...
    def fit(self, data):
        ...
        for col, disc in enumerate(self.discs):
            ...
            disc.fit(fit_data)
            self.cat_cards.append(int(disc.n_bins_ + 1))

    def enc_cat(self, data):
        # todo: add support for missing values, which should get encoded as 0
        data = LatLongScalarEnc().enc_cont(data)
        feats = []
        for col, disc in enumerate(self.discs):
